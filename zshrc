...
if [ "$color_prompt" = yes ]; then
    ...
    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)──}$(venv_info)(%B%F{%(#.cyan.blue)}%n%(#.💀.㉿)%m%b%F{%(#.blue.green)})─(%F{reset}$CURRENTIP%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]\n└─%B%(#.%F{blue}#.%F{blue}$)%b%F{reset} '
